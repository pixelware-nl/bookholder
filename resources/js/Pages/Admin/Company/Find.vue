<template>
    <AdminContainer :form-title="$t('company.find.title')" :route-name="route('companies.index')">
        <h2 class="mb-2 text-xl font-bold uppercase">{{ $t('company.find.search_kvk') }}</h2>
        <form @submit.prevent="kvkForm.post(route('companies.found'))">
            <InputContainer>
                <TextInput id="kvk" name="kvk_to_find" v-model="kvkForm.kvk_to_find" label="KVK" placeholder="12345678" :error="errors.kvk_to_find" />
            </InputContainer>
            ...
            <div v-if="errors.kvk_to_find">
                ...
                <p class="mt-6 w-full text-center">
                    ...
                    <Link :href="route('companies.create', kvkForm.kvk_to_find)" class="text-blue-600 hover:underline">{{
                        $t('company.find.manual_create_link')
                    }}</Link>
                </p>
            </div>
        </form>

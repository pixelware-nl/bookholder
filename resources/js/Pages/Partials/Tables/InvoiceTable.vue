<template>
    <TableContainer>
        ...
        <template #tbody v-if="filteredInvoices.length > 0">
            ...
            >
                ...
                <td class="table-item table-item-link">
                    <Link
                        v-if="invoice.payed == false"
                        :href="route('invoices.payed', invoice.id)"
                        method="post"
                    >
                        <FontAwesomeIcon
        ...
        <template #tbody v-else>
            <tr>
                <td colspan="10" class="text-center !text-gray-400">
                    {{ $t("invoice.index.no_entries") }}
                </td>
            </tr>
        </template>
    </TableContainer>
